...
class Output(object):
    def __init__(self):
        ...
        # 初始化日志记录器
        self.logger = logging.getLogger('WebAliveScan')
        self.logger.setLevel(getattr(logging, config.log_level.upper()))
        
        # 创建日志格式
        formatter = logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s')
        
        # 创建文件处理器，使用RotatingFileHandler来限制文件大小
    ...
    def statusReport(self, url_info):
        ...
        with self.mutex:
            self.scanned += 1
            # 计算进度百分比
            percentage = int((self.scanned / self.total) * 100) if self.total > 0 else 0
            # 显示进度条
            progress_bar = "█" * (percentage // 2) + "░" * (50 - percentage // 2)
            progress_str = f"[{progress_bar}] {percentage}% ({self.scanned}/{self.total})"
    ...
    def config(self, threads, list_size):
        ...
        self.newLine(config)
        # 记录到日志
        self.logger.info(f'Threads: {threads} | Number of requests: {list_size}')
    ...
    def bruteTarget(self, target):
        ...
        self.newLine(config)
        # 记录到日志
        self.logger.info(f'DirBrute Target: {target}')
